<#	
	...
	===========================================================================
	 ...
	 Filename:     	Get-UserGPOs.ps1
	===========================================================================
	.SYNOPSIS
        PowerShell scanner for PDQ Inventory which returns group policy results within the user scope
    .DESCRIPTION
        This script is a PowerShell scanner for PDQ Inventory which finds group policies within the user scope via RSOP and returns the data as a PowerShell object array
    .EXAMPLE
        .\Get-UserGPOs.ps1
    .INPUTS
        None.
...
function New-ResultantSetOfPolicyReport {
    Start-Process -FilePath "$env:SystemRoot\System32\gpresult.exe" -ArgumentList "/Scope:User", "/x $Path", "/f" -WindowStyle Hidden -Wait
    [xml] $XML = Get-Content -Raw $Path
    return ($XML.documentelement.userresults.GPO | Where-Object { $_.Enabled -eq $true })
}
